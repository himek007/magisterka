...
def predict():
    ...
    label_las_czas.configure(text = "{:.5f}".format(czasLas1)+"s")
    czasDrzewo1 = float(czasDrzewo.split(":")[1].strip()[:-1])
    label_drzewo_czas.configure(text = "{:.5f}".format(czasDrzewo1)+"s")
    czasNeuro1 = float(czasNeuro.split(":")[1].strip()[:-1])
    label_neuro_czas.configure(text = "{:.5f}".format(czasNeuro1)+"s")
...
label = customtkinter.CTkLabel(master=frame, text="System wyboru przedmiotu zaopatrzenia medycznego po udarze")
label.pack(pady=12, padx=10)
plec1 = customtkinter.CTkOptionMenu(frame,values=["Mezczyzna", "Kobieta"])
plec1.pack(pady=12, padx=10)
...
wiek1 = customtkinter.CTkEntry(master= frame, placeholder_text="Wpisz wiek")
wiek1.pack(pady=12, padx=10)
def set_wiek():
    wiek = wiek1.get()
    return wiek

tygodnie1 = customtkinter.CTkEntry(master= frame, placeholder_text="Ile tygodni po udarze")
tygodnie1.pack(pady=12, padx=10)
def set_tygodnie():
    tygodnie=tygodnie1.get()
    return tygodnie
button = customtkinter.CTkButton(master=frame, text="Przewidywanie", command=predict)
button.pack(pady=12, padx=10)

label_las = customtkinter.CTkLabel(master=frame, text="Losowy Las")
label_las.pack(pady=12, padx=10)
label_las_czas = customtkinter.CTkLabel(master=frame, text="Czas treningu:")
label_las_czas.pack(pady=12, padx=10)
label_las_cel = customtkinter.CTkLabel(master=frame, text="Celność:")
label_las_cel.pack(pady=12, padx=10)
label_las_wynik = customtkinter.CTkLabel(master=frame, text="Wynik przewidywania:")
label_las_wynik.pack(pady=12, padx=10)

label_drzewo = customtkinter.CTkLabel(master=frame, text="Drzewo Decyzyjne(C&RT)")
label_drzewo.pack(pady=12, padx=10)
label_drzewo_czas = customtkinter.CTkLabel(master=frame, text="Czas treningu:")
label_drzewo_czas.pack(pady=12, padx=10)
label_drzewo_cel = customtkinter.CTkLabel(master=frame, text="Celność:")
label_drzewo_cel.pack(pady=12, padx=10)
label_drzewo_wynik = customtkinter.CTkLabel(master=frame, text="Wynik przewidywania:")
label_drzewo_wynik.pack(pady=12, padx=10)

label_neuro = customtkinter.CTkLabel(master=frame, text="Sieć neuronowa")
label_neuro.pack(pady=12, padx=10)
label_neuro_czas = customtkinter.CTkLabel(master=frame, text="Czas treningu:")
label_neuro_czas.pack(pady=12, padx=10)
label_neuro_cel = customtkinter.CTkLabel(master=frame, text="Celność:")
label_neuro_cel.pack(pady=12, padx=10)
label_neuro_wynik = customtkinter.CTkLabel(master=frame, text="Wynik przewidywania:")
label_neuro_wynik.pack(pady=12, padx=10)
